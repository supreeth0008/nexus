...
class DiagnosisEngine:
    def diagnose(self, incident: Incident, signals: list) -> Incident:
        ...
        incident.root_cause = (
            ...
            or "unknown – automated correlation found no clear change"
        )
        # blend confidence
    ...
    def _correlate(self, incident: Incident, signals: list) -> tuple[str, float]:
        ...
        text_blob = " ".join([
            getattr(s, "name", "") + " " + str(getattr(s, "value", ""))
            for s in signals
        ]).lower()
        ...
        for kw, cause, conf in rules:
            if (
                kw in text_blob
                or kw in incident.source_signal.lower()
                or kw in incident.target_id.lower()
            ):
                return cause, conf
        # I fall back to incident type mapping
        type_map = {
            "performance_degradation": (
                "Performance regression – resource bottleneck suspected",
                0.6,
            ),
            "reliability_degradation": (
                "Reliability drop – error rate / crashloop",
                0.65,
            ),
            "security_drift": (
            ...
            "cost_spike": (
                "Unexpected cost increase – scaling or new resource",
                0.6,
            ),
            "compliance_drift": (
                "Compliance tag / region policy violation",
                0.65,
            ),
            "scaling_bottleneck": (
                "Scaling limit hit – HPA / ASG max reached",
                0.75,
            ),
            "resource_exhaustion": (
